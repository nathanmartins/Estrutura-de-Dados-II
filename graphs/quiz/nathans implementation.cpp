...
void VerticesSemAdjacencia(TipoGrafo* Grafo) {
    /* 
        Roda uma  variante do DFS e verifica quem não foi visitado ainda.
        Imprime esses que não foram visitados. 
        Depois zerar todos os campos como o DFS original faz.
    */
    

    printf("\n--------------------------------------------");

    printf("\nVertices sem adjacencias: ");
    ...
    for (int i = 0; i < 8; ++i)
    {
        ...
        TipoVertice* v  = &(Grafo->ListaVertices[i]);
...
void RemoverAresta(TipoGrafo* Grafo, int inicio, int fim) {
    // Faz o oposoto que o adicionarAresta faz KKKKKKKKKKKK
	Grafo->MatrizAdj[inicio][fim] = 0;
    Grafo->MatrizAdj[fim][inicio] = 0;
}
