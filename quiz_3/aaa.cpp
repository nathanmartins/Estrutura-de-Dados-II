...
void Busca(TipoChave Chave, char *nomeAluno, TipoTabela *T) {
    ...
    if (T->Item[indice].Chave != -1) {
        ...
        TipoIndice passo = 5 - (T->Item[indice].Chave % 5);
        do {
            ...
            if (comparaTexto(T->Item[indice].Nome, nomeAluno)) {
                printf("\n");
                printf("Registro Encontrado:\n");
                printf("Chave: %d\nNome: %s\nIdade: %d", T->Item[indice].Chave, T->Item[indice].Nome,
                       T->Item[indice].Idade);
                printf("\n");
                return;
            }

            indice = (indice + passo) % MAXN;

        } while (T->Item[indice].Nome != nomeAluno);
